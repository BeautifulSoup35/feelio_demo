...
function MoodChip({ label, color, selected = false, removable = false, muted = false, onClick }) {
  const Component = onClick ? 'button' : 'span';

  return (
    <Component
      type={onClick ? 'button' : undefined}
      className={`moodChip ${selected ? 'selected' : ''} ${muted ? 'muted' : ''}`}
...
export default function EmotionExpenseCard({ onSubmit }) {
  ...
  const activePickerConfig = useMemo(() => {
    if (activePicker === 'category') {
      return { chips: categoryChips, selectedIds: [selectedCategoryId], onSelect: setSelectedCategoryId };
    }
    if (activePicker === 'emotion') {
      return { chips: emotionChips, selectedIds: [selectedEmotionId], onSelect: setSelectedEmotionId };
    }
  ...
  const isReadyToSave = parseMoney(amount) > 0 && memo.trim().length > 0;
    ...
    <section className="emotionExpenseCard" aria-label="감정 소비 태그 선택">
      <div className="emotionExpenseHeader">
        ...
        <div className="homeActionCluster">
          ...
          {isReadyToSave && (
            <button type="button" className="expenseSaveButton" onClick={saveRecord}>
              저장
            </button>
          )}
        </div>
      ...
      <div className="expensePreviewCard">
        ...
        <div className="selectedMoodTags">
          <div className="pickerCategoryChips inline" aria-label="태그 카테고리">
            {pickerGroups.map(group => (
              <button
                ...
                onClick={() => setActivePicker(prev => prev === group.id ? null : group.id)}
              >
                {group.selected && <i />}
                {group.label}
              </button>
          ...
          <span className="chipCaret" aria-hidden="true" />
        </div>
      </div>
